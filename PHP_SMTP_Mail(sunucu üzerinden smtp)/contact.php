...
$TelefonNumarasi = trim(strip_tags($_POST['mobile']));
$Mesaj = trim(strip_tags($_POST['comment']));
//Form'dan Bütün Değerler Post Methodu ile Çekiliyor Tamamlandı


if($AdiSoyadi and $MailAdresi and $MesajKonusu and $Mesaj and $TelefonNumarasi){ //Form'dan bütün değerler geliyorsa mail gönderme işlemini başlatıyoruz.
    ...
    $MailSmtpHost = "ni-kyrenia.yourhosting.com";//Burada hostinginizin ayarlarında bulabileceğiniz mail gönderme uzantısını kullanın
    $MailUserName = "info@yourdomain.com";
    
    //Mail Bağlantı Ayarları Tamamlandı

    //Doldurulan Form Mail Olarak Kime Gidecek?
    $MailKimeGidecek = "support@yourdomain.com";
    ...
    $mail->SetFrom($mail->Username, 'Mail Bilgileri');
    $mail->AddAddress("$MailKimeGidecek", 'Alıcı adı'); //Mailin Gideceği Adres ve Alıcı Adı
    $mail->CharSet = 'UTF-8'; //Mail Karakter Seti
    $mail->Subject = $MesajKonusu; //Mail Konu Başlığı
    $mail->MsgHTML("$Mesaj"); //Mail Mesaj İçeriği
    if($mail->Send()) {
        
     echo '<script>document.location="mail.php"</script>';
    } else {
